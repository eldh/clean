...
DatepickerMonth =  React.createFactory require './DatepickerMonth'
...
describe 'date range with no date selected and firstSelectableDate, fromMonth', ->
	before (done) ->
		...
		render DatepickerMonth(
			timeperiod: timeperiod
			isFromMonth: true
			firstSelectableDate: moment().startOf 'month'
			onDateSelected: -> null
		)
		, document.body
		done()
	...
	it 'should render next btn as enabled', ->
		document.querySelectorAll('.datepicker__next--disabled')
			.length
			.should.equal 0
describe 'date range with no date selected and firstSelectableDate, toMonth', ->
	before (done) ->
		timeperiod = {}
		render DatepickerMonth(
			timeperiod: timeperiod
